...
fn random_scene(rng: &mut SmallRng) -> HitableList{
	...
	objs.push(Box::new(Sphere::new(Vec3::new(0.0, -1000.0, 0.0), 1000.0, Box::new(Lambertian::new(Colour::new(0.5, 0.5, 0.5))))));
		...
		for b in -11..11{
			...
			if (center - Vec3::new(4.0, 0.2, 0.0)).length() > 0.9 {
				if choose_mat < 0.8 {	//diffuse
					...
					objs.push(Box::new(Sphere::new(center, 0.2, Box::new(Lambertian::new(Colour::new(rng.gen::<f32>()*rng.gen::<f32>(), rng.gen::<f32>()*rng.gen::<f32>(), rng.gen::<f32>()*rng.gen::<f32>()))))));
				} else if choose_mat < 0.95 { // metal
					objs.push(Box::new(Sphere::new(center, 0.2, Box::new(Metal::new(Colour::new(0.5*(1.0+rng.gen::<f32>()), 0.5*(1.0+rng.gen::<f32>()), 0.5*(1.0+rng.gen::<f32>())), 0.0)))));
				} else { //glass
					objs.push(Box::new(Sphere::new(center, 0.2, Box::new(Dielectric::new(1.5)))));
				}
			}
		}
	}
	objs.push(Box::new(Sphere::new(Vec3::new(0.0, 1.0, 0.0), 1.0, Box::new(Dielectric::new(1.5)))));
	objs.push(Box::new(Sphere::new(Vec3::new(-4.0, 1.0, 0.0), 1.0, Box::new(Lambertian::new(Colour::new(0.4, 0.2, 0.1))))));
	objs.push(Box::new(Sphere::new(Vec3::new(4.0, 1.0, 0.0), 1.0, Box::new(Metal::new(Colour::new(0.7, 0.6, 0.5), 0.0)))));
