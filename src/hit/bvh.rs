...
impl BvhNode {
	...
	pub fn new(mut objects: Vec<Box<dyn Hitable>>) -> BvhNode{
		...
		let comparator = match axis {
			...
			_ => {panic!("Got a number out of range! Recieved: {}", axis);}
		};
	...
	pub fn new_sah(mut l: Vec<Box<dyn Hitable>>) -> BvhNode{
		...
		assert!(left_area.len() == n-1, "left area length was {}, but should have been {}", left_area.len(), n-1);
		...
		assert!(left_area.len() == n-1, "right area length was {}, but should have been {}", right_area.len(), n-1);		
